...
from .qt_offscreen import OffscreenRenderer as _OffscreenRenderer
...
class NCViewer(tk.Tk):
    def __init__(self):
        ...
        # Qt 离屏渲染
        self._qapp = _QApplication.instance() or _QApplication([])
        self._renderer = None            # OffscreenRenderer (延迟创建)
        self._qt_ready = False
        self._photo = None               # 当前显示的 PhotoImage
        self._img_item = None            # canvas 图片项
        self.after(150, self._init_qt_render)
    ...
    def _init_qt_render(self):
        """创建离屏渲染器 (QApplication 已存在)"""
        try:
            self._renderer = _OffscreenRenderer()
            self._qt_ready = True
        except Exception as ex:          # OpenGL 不可用时回退 Tk 渲染
            self._qt_ready = False
            self._renderer = None
            self.status.config(text="Qt 离屏渲染初始化失败: %s" % ex)
            return
        if self.result:
            self.render()
    ...
    def _qt_render(self):
        ...
        img = img.convertToFormat(_QImage.Format_RGB888)
        bpl = img.bytesPerLine()
        data = bytes(img.bits().asstring(bpl * H))
        from PIL import Image, ImageTk
        pil = Image.frombuffer("RGB", (W, H), data, "raw", "RGB", bpl, 0)
        if self._photo is None or self._photo.width() != W \
                or self._photo.height() != H:
            # 画布尺寸变化时重建 PhotoImage
            self._photo = ImageTk.PhotoImage(pil)
        else:
            # 复用 PhotoImage 更新像素: 播放每 tick 新建会累积 Tk 图片
            # 对象 (pyimageN), 越用越卡
    ...
    def _on_canvas_configure(self, e):
        """画布尺寸变化: [Qt 离屏] 图片需按新尺寸重渲染, 并重贴图例"""
        self._place_legend()
        if self._qt_ready and self.result:
            self._qt_render()
    ...
    # ------------- 代码列表 -------------
    def _fill_code(self):
        self._move_by_line = {}          # 供离屏 overlay 高亮当前行
        if not self.result:
        ...
        for m in self.result.moves:
            self._move_by_line.setdefault(m.line_number, m)
        self.code.configure(state="normal")
        self.code.delete("1.0", "end")
        # 为每个颜色建一个标签
    ...
    def zoom_at(self, factor, anchor):
        ...
        self.offset = (ox, oy)
        if self.result:
            # [Qt 离屏] 缩放 = 按新 scale/offset 重渲染
            # (canvas.scale 对图片项无效, 且标记由 overlay 绘制)
            self._qt_render()

    def render(self):
        """[Qt 离屏] 渲染由 Qt 离屏完成; 数据变化时重建 VBO, 否则只渲染"""
        ...
        if animate:
            if self._trace_active:
                if self._play_mode == "play":
                    # 播放: _trace_drawn 已由 _play_tick 按移动维护
                    self._qt_render()
                else:
                    self._trace_draw_to_line(ln)   # 单步/直达: 行级
            else:
                self._qt_render()
        else:
            self.render()
    ...
    def _play_tick(self):
        if self._play_mode != "play":
            return
        # [Qt 离屏] 移动级渐进: 刀路按移动逐步增长 (走到哪显示到哪)。
        # 推进语义保持行级 (合并跳行 N 行, 段模式钳制在段内), 渲染按
        # 移动裁剪: 目标行 -> 该行最后移动索引 -> 离屏渲染 trace_drawn
        if not self._trace_active:
            self._trace_begin()
        lo, hi = self._seg_line_bounds()
        base_line = self.current_line if self.current_line else lo - 1
        if base_line >= hi:
    ...
    def _trace_draw_to(self, k):
        ...
        (原 Tk 版在此画 canvas 折线图元, 会与离屏图片叠加残留)"""
