...
def test_draw_all_draws_full_path(app, tmp_path):
    ...
    # [Qt 离屏] 前导跳过 1 移动, 其余全部画出 (trace_drawn = 全部移动数)
...
def test_tool_model_draws_and_toggles(app, tmp_path):
    """[Qt 离屏] 刀具 3D 模型由 overlay 绘制: 定位触发渲染, 开关关闭后不绘"""
    import time
    p = tmp_path / "t.nc"
    p.write_text("G01X10Y20F1000\nX20Y30\n", encoding="utf-8")
    app.open_file(str(p))
    ...
    for _ in range(10):                 # 等待离屏渲染器就绪 (after 150ms)
        app.update()
        time.sleep(0.05)
    ...
    assert app._renderer._show_tool and app._renderer._tool is app.tool
    app.show_tool.set(False)
    app.render()
    assert not app._renderer._show_tool, "关闭后离屏 overlay 不再绘刀具"
...
def test_dimension_arrows_point_outward(app):
    ...
    cv.destroy()
    cv2.destroy()
def test_tool_model_solid_body(app, tmp_path, monkeypatch):
    """[Qt 离屏] 3D 刀具模型由 Qt overlay 绘制 (实体 GL 轮廓, 无半透明);
    定位触发离屏重渲染"""
    p = tmp_path / "t.nc"
    p.write_text("G01X10Y20F1000\nX20Y30\n", encoding="utf-8")
    app.open_file(str(p))
    ...
    calls = []
    orig = app._qt_render
    monkeypatch.setattr(app, "_qt_render",
                        lambda: (calls.append(1), orig()))
    app.set_current_line(1)
    assert calls, "定位应触发离屏重渲染 (含 3D 刀具 overlay)"
    assert app._renderer._tool is app.tool
    assert app._renderer._show_tool


def test_tool_model_no_oversize_when_large(app, tmp_path, monkeypatch):
    """[Qt 离屏] 3D 刀具模型各视角均触发离屏重渲染 (放大判定在离屏渲染器,
    直径投影角度无关不突然放大)"""
    ...
    for q in (VIEW_QUAT["XY"], VIEW_QUAT["XZ"], VIEW_QUAT["YZ"]):
        ...
        app.quat = q
        app.render()
        assert calls, "视角切换应触发离屏重渲染 (刀具 overlay 随视角重绘)"
    app.quat = (1.0, 0.0, 0.0, 0.0)      # 共享 fixture: 恢复默认视图


def test_tool_model_zoomed_when_tiny(app, tmp_path, monkeypatch):
    """[Qt 离屏] 3D 刀具模型: 定位/适配触发离屏重渲染 (小刀具放大判定在
    离屏渲染器, 直径投影 <24px 时放大到可见尺寸)"""
    app.state("normal")
    app.geometry("1280x800+50+50")
    app.update()
    p = tmp_path / "t.nc"
    ...
    app.set_current_line(1)
    app.fit_view()
    assert calls, "定位/适配应触发离屏重渲染 (含刀具 overlay)"


def test_tool_tip_anchored_at_position(app, tmp_path, monkeypatch):
    """[Qt 离屏] 刀尖对刀由 Qt overlay 锚定 (刀尖 = 当前执行位置);
    设定刀具并定位时触发离屏重渲染"""
    p = tmp_path / "t.nc"
    p.write_text("G01X10Y20F1000\n", encoding="utf-8")
    app.open_file(str(p))
    ...
    monkeypatch.setattr(app, "_qt_render",
                        lambda: (calls.append(1), orig()))
    app.set_current_line(1)
    assert calls, "定位应触发离屏重渲染 (刀尖锚定当前执行位置)"
...
def test_segment_mode_renders_only_current_segment(app, tmp_path):
    ...
    # 播放钳制在段内 ([Qt 离屏] 移动级推进, 从段首起, 到段尾停止)
    app._reset_line()
    assert app.current_line == 1
    app._play_mode = "play"
    app._play_tick()
    assert 2 <= app.current_line <= 4, "段内推进 (移动级)"
    for _ in range(10):
        app._play_tick()
        if app._play_mode is None:
            break
    assert app._play_mode is None, "到段尾应停止"
...
def test_seg_filter_change_during_trace_full_render(app, tmp_path):
    ...
    assert app._trace_active
    assert app._trace_drawn >= 1, "段1 已画 ([Qt 离屏] 按移动裁剪渲染)"
    # 模拟列表点击追加勾选第 5 段
    app.seg_listbox.selection_set(4)
    app._on_seg_list_select(None)
    assert not app._trace_active, "过滤变化后应退出轨迹模式做全量渲染"
    assert app._seg_filter == [(0, 2), (12, 14)]   # 段1 + 段5 并集 (移动索引)
    app._seg_only.set(False)
    app._toggle_seg_only()
...
def test_crosshair_respans_and_dimmed(app, tmp_path, monkeypatch):
    """[Qt 离屏] 十字线/当前点由 Qt overlay 绘制 (canvas 无图元);
    平移触发离屏重渲染 (十字线随可视区重铺)"""
    p = tmp_path / "t.nc"
    p.write_text("G01X10Y20F1000\nG01X30Y40\n", encoding="utf-8")
    app.open_file(str(p))
    ...
    app.update()
    app.set_current_line(2)
    # overlay 在 Qt 侧: 画布不再有十字虚线/当前点图元
    assert not app.canvas.find_withtag("curx")
    ...
    assert calls, "平移应触发离屏重渲染 (十字线按新可视区重铺)"
...
def test_canvas_click_empty_clears_selection(app, tmp_path):
    ...
    assert app.current_line is None
    # [Qt 离屏] overlay 由 Qt 绘制, canvas 无标记图元; 状态清除即可
    assert not app.canvas.find_withtag("cur")
    assert not app.canvas.find_withtag("curseg")
...
def test_direction_arrows_fixed_size_after_zoom(app, tmp_path, monkeypatch):
    """[Qt 离屏] 方向箭头由 Qt overlay 按固定像素尺寸 (NDC 14px) 绘制;
    画布无箭头图元, 缩放触发离屏重渲染"""
    p = tmp_path / "t.nc"
    p.write_text("G01X10Y0F1000\nG01X200Y0\n", encoding="utf-8")
    app.open_file(str(p))
    ...
    app.fit_view()
    app.set_current_line(2)
    # overlay 在 Qt 侧: canvas 无多边形箭头图元
    assert not [i for i in app.canvas.find_withtag("cur")
    ...
    assert calls, "缩放应触发离屏重渲染 (箭头固定尺寸由 overlay 保证)"
...
def test_canvas_resize_keeps_path_items(app, tmp_path):
    """[Qt 离屏] 画布尺寸变化: 复用图片项, PhotoImage 按新尺寸重建重渲染"""
    import time
    p = tmp_path / "t.nc"
    p.write_text(NC_SMALL, encoding="utf-8")
    app.open_file(str(p))
    ...
    for _ in range(10):                 # 等待离屏渲染器就绪 (after 150ms)
        ...
        if getattr(app, "_qt_ready", False):
            break
    app.update()
    assert app._img_item is not None
    before = app._img_item
    old_size = (app._photo.width(), app._photo.height())
    app.geometry("1400x860+100+100")
    for _ in range(5):
        app.update()
        time.sleep(0.05)
    assert app._img_item == before, "尺寸变化应复用图片项 (不 delete/create)"
    assert (app._photo.width(), app._photo.height()) != old_size, \
        "PhotoImage 应按新画布尺寸重建重渲染"


def test_rotation_reuses_path_items(app, tmp_path):
    """[Qt 离屏] 旋转复用画布图片项与 PhotoImage (重渲染不 delete/create)"""
    import time
    p = tmp_path / "t.nc"
    p.write_text(NC_SMALL, encoding="utf-8")
    app.open_file(str(p))
    ...
    assert app._img_item is not None
    before = app._img_item
    photo_before = app._photo
    app.quat = orbit_rotate(app.quat, 30, 20)
    app.render()
    assert app._img_item == before, "旋转应复用画布图片项而非重建"
    assert app._photo is photo_before, "旋转应复用 PhotoImage 而非新建"
...
def test_lead_skip_skips_origin_approach(app, tmp_path):
    ...
    app.update()
    # [Qt 离屏] 轨迹模式从跳过处开始 (已执行移动数 = 前导跳过数)
    app._trace_begin()
    assert app._trace_drawn == 2
...
def test_trace_pan_syncs_stored_coords(app, tmp_path):
    """[Qt 离屏] 轨迹模式下平移: 已执行移动数保持, 平移后追加不崩溃"""
    from types import SimpleNamespace
    p = tmp_path / "t.nc"
    p.write_text(NC_SMALL, encoding="utf-8")
    app.open_file(str(p))
    app._trace_begin()
    app.set_current_line(2, animate=True)     # 首段(从原点)被跳过, 从第2移动开始
    drawn = app._trace_drawn
    app._pan_start(SimpleNamespace(x=0, y=0))
    app._pan_move(SimpleNamespace(x=7, y=3))
    assert app._trace_drawn == drawn, "平移不改变已执行移动数 (画面由离屏重渲染)"
    # 平移后继续追加不崩溃
    app.set_current_line(3, animate=True)
    assert app._trace_drawn >= drawn


def test_trace_draws_progressively(app, tmp_path):
    """[Qt 离屏] 轨迹渐进: 从空白起按移动逐步画刀路, 后退重绘

    渲染裁剪由离屏渲染器按已执行移动数完成, 此处验证状态推进:
    首段(从原点出发)被前导跳过, 轨迹从第 2 个移动开始画。
    """
    p = tmp_path / "t.nc"
    p.write_text(NC_SMALL, encoding="utf-8")
    app.open_file(str(p))
    ...
    app._trace_begin()
    assert app._trace_drawn == 1         # 前导跳过第 1 移动 (原点出发)
    app.set_current_line(1, animate=True)     # 第1移动被跳过 -> 不变
    assert app._trace_drawn == 1
    app.set_current_line(2, animate=True)     # G2 F2000 -> 推进
    assert app._trace_drawn == 2
    app.set_current_line(3, animate=True)     # G0 灰色 -> 推进
    assert app._trace_drawn == 3
