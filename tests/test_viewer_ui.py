...
def test_sidebar_bottom_three_pinned(app):
    """程序统计/当前位置/刀具三区贴住侧栏底部 (滚动区只剩图例)"""
    inner = app._side_inner
    assert app.legend.master is inner                     # 图例在滚动区
    side = app.stats_labels["x"].master.master
    assert side is app.pos_fields["X"].master.master      # 统计/位置同容器
    assert side is app.tool_cv.master.master              # 刀具同容器
